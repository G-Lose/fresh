...
export default {
  data: function() {
    return {
      goodlist: [{}],
      checkedgou: -1,
      checkedab: false, //全选反选控制
      istrue: false, //控制筛选div
      inpchecked: false, //检测是否有选中
      checkedname: [], //获取选中项渲染到list
      filename: [] //存放全选时类别名字
    };
  },
  watch: {},
  methods: {
    ...
    checkall() {
      this.checkedab = !this.checkedab;
      if (this.checkedab) {
        this.checkedname = this.filename;
        PubSub.publish("ckname", this.checkedname);
      } else {
        this.checkedname = [];
        PubSub.publish("ckname", this.checkedname);
      }
  ...
  async created() {
    ...
    for (var i = 0; i < 13; i++) {
      this.filename.push(children[0].children[i].name);
    }
